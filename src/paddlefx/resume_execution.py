...
def _filter_iter(l1, l2, cond):
    ...
    e.g. _filter_iter(insts, sorted_offsets, lambda i, o: i.offset == o) returns the
    instructions with offsets in sorted_offsets
    """
    it = iter(l2)
    res = []
    try:
...
class ContinueExecutionCache:
    ...
    def generate_based_on_original_code_object(
        ...
        to resume something else.

        We want to always generate starting from the original code object so that if
        control flow paths converge we only generated 1 resume function (rather than
        2^n resume functions).
        """
